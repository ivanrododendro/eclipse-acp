...
5. In Project Explorer, right-click a project or any child resource and choose **ACP → Open ACP Session**. The session always uses the owning project directory. Use `Ctrl+Alt+O` (`⌘⌥O` on macOS) to open a session for the selected project or active editor's project.
